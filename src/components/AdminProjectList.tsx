...
export default function AdminProjectList({ locale, initialProjects, initialPortfolio }: AdminProjectListProps) {
  ...
  const handleEditProject = (project: Project) => {
    ...
    setEditForm({ projectName: project.projectName, projectType: project.projectType, title: "", description: "" });
  };
  ...
  const handleSaveProject = async (id: number) => {
    const response = await fetch("/api/edit", {
      ...
      body: JSON.stringify({ id, projectName: editForm.projectName, projectType: editForm.projectType }),
    });

    if (response.ok) {
      setProjects(
        projects.map((project) =>
          project.id === id ? { ...project, projectName: editForm.projectName, projectType: editForm.projectType } : project
        )
      );
      setEditingId(null);
    }
  ...
  const handleEditPortfolio = (item: PortfolioItem) => {
    ...
    setEditForm({ projectName: "", projectType: "", title: item.title, description: item.description || "" });
  };
          ...
          <li key={project.id} className="mb-4 p-4 border rounded flex justify-between items-center">
            {editingId === project.id ? (
              ...
              <>
                ...
                <button onClick={() => handleEditProject(project)} className="px-3 py-1 bg-yellow-600 text-white rounded">
          ...
          <li key={item.id} className="mb-4 p-4 border rounded flex justify-between items-center">
            {editingId === item.id ? (
              ...
              <>
                ...
                <button onClick={() => handleEditPortfolio(item)} className="px-3 py-1 bg-yellow-600 text-white rounded">
                  {locale === "fa" ? "ویرایش" : "Edit"}
                </button>
              </>
            )}
